...
function ensureUnifiedPageScrollStyles() {
  ...
  style.id = 'unifiedPageScrollStyles';
  style.textContent = `
    html,
    body {
...
function ensureAllDaysQuickViewStyles() {
  if (document.getElementById('allDaysQuickViewStyles')) return;

  const style = document.createElement('style');
  style.id = 'allDaysQuickViewStyles';
  style.textContent = `
    .all-days-quick-tile {
      max-width: 720px;
      width: 100%;
...
function bindAutoWeekLoader() {
  if (window.__autoWeekLoaderBound || typeof state === 'undefined') return;
  ...
  function selectWeek(date) {
    ...
    if (state.savedWeeks && state.savedWeeks[key]) {
      loadWeekSnapshot(key);
      setStatus('Saved week loaded');
      setTimeout(updateAllDaysQuickViewTile, 0);
      return;
    }
    ...
    setStatus('New blank week started');
    setTimeout(updateAllDaysQuickViewTile, 0);
  }
...
function bindDelegatedModalEvents() {
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape') closeAllDaysQuickModal();
...
const TrackerModal = (() => {
  const el = {
    backdrop: document.getElementById('trackerModalBackdrop'),
    closeBtn: document.getElementById('closeTrackerModalBtn'),
    content: document.getElementById('trackerModalContent')
  };

  function open(html = '') {
    if (el.content) el.content.innerHTML = html;
    if (el.backdrop) {
  ...
  function renderRow(left, middle, right, extraClass = '') {
    return `
  ...
  if (el.backdrop) {
    el.backdrop.addEventListener('click', event => {
      if (event.target === el.backdrop) close();
    });
  }

  return { open, close, renderSection, renderRow };
})();
